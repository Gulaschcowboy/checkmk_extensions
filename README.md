...
| [`monitoring_compliance`](monitoring_compliance/) | Capability-based detection of installed/running host subsystems that could be monitored by Checkmk but aren't yet — correlates agent sections, systemd/Windows services, processes, host labels and HW/SW inventory packages against available check plug-ins, with a persistent capability database and an informational known-catalog reference service. |
